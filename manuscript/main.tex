...
\begin{enumerate}
    \item an artifact-driven methodology that adapts and integrates established methods from metadata profile development, schema discovery, ontology learning, knowledge graph construction, and multimodal document understanding to derive an operational metadata schema from the artifacts it must describe;
...
As illustrated in Figure~\ref{fig:conceptual_relationship}, schema design produces the operational representation to be evaluated. Schema validation completes the methodology by assessing that representation against held-out artifacts and incorporating justified revisions. The endpoint is therefore an operational metadata schema accompanied by held-out validation evidence and a record of human adjudication decisions. Metadata extraction and downstream applications fall outside the scope of this work, as do subsequent activities that adapt the schema to specific technical or interoperability requirements.
...
The stages separate artifact-level discovery, field profile aggregation, canonical metadata concept synthesis, expert concept refinement, concept coverage assessment, metadata schema design and specification, and schema validation. This decomposition makes each transformation independently reviewable and preserves traceability from the operational schema to the artifacts that motivated it. Table~\ref{tab:methodology_summary} summarizes the workflow.
...
\begin{figure}[!ht]
    ...
    \caption{The development workflow transforms the 210 development snapshots into the Data Snapshot Metadata Schema through explicit intermediate knowledge artifacts and evaluates the schema against held-out snapshots. Colors distinguish LLM-supported, deterministic, and human-guided transformations.}
...
The methodological contribution is the adaptation and integration of established principles from metadata profile development, schema discovery, ontology learning, knowledge graph construction, and multimodal document understanding into an evidence-preserving sequence that transforms artifact-level observations into an operational metadata schema and then evaluates that schema against held-out artifacts. The Data Snapshot Metadata Schema provides a case through which this architecture is examined; evaluating its transfer to other artifact classes and development settings remains future work.
...
Evaluate the attached snapshot using the system instructions. Return the structured schema-validation assessment only.
\end{lstlisting}
...
\begin{lstlisting}
# Frozen Data Snapshot Metadata Schema v1.1.1
