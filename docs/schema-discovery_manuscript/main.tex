...
\begin{figure}[!ht]
    ...
    \caption{Case-study execution of the proposed methodology. The workflow transforms 210 development snapshots into an initial 36-field schema, with concept coverage assessment applied to the refined concept inventory and schema validation performed on a separate collection of 202 held-out snapshots. Human revision produces the validated 36-field schema, which is then refined against authoritative standards and vocabularies to produce the final 37-field Data Snapshot Metadata Schema. Colors distinguish LLM-supported, deterministic, and human-guided transformations.}
