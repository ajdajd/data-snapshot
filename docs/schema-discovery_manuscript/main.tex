...
The crosswalk classified the relationship between each field and the reviewed standards rather than assuming that every local concept required a direct equivalent. Exact or close mappings were available for 21 fields. Nine fields were narrower or broader than the closest standard term, one field combined concepts that the standards distinguished structurally, and five retained locally defined semantics because no direct cross-domain match was identified.
...
The standards crosswalk introduced a different form of evidence after coverage validation. It did not ask whether an artifact required an additional metadata concept. Instead, it compared the 36 validated fields with established terminology and representation patterns (Table~\ref{tab:standards_crosswalk_results}). Ten fields had exact mappings and eleven had close mappings. For the remaining fields, the closest standard was broader, narrower, structurally related, or absent. The results therefore supported selective reuse across multiple standards rather than adoption of a single external vocabulary.
