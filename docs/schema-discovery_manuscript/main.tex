...
Developing an operational metadata schema for a multimodal artifact class requires identifying its descriptive requirements before metadata extraction can begin. We present an artifact-driven, human-in-the-loop methodology that adapts principles from metadata profile development, schema discovery, ontology learning, knowledge graph construction, and multimodal document understanding. The methodology decomposes schema development into explicit, evidence-preserving semantic transformations connected by intermediate knowledge artifacts. Multimodal large language models (LLMs) support artifact interpretation, concept synthesis, constrained semantic comparison, and standards research, while human experts determine conceptual boundaries, operational fields, validation decisions, and standards-informed refinements. We apply the methodology to 210 data snapshots extracted from institutional documents, producing an initial 36-field schema, evaluating it against a separate collection of 202 snapshots, and refining it into the 37-field Data Snapshot Metadata Schema. Human adjudication accepted no additional metadata requirement during held-out validation while identifying documentation ambiguities and representation limitations. The contribution is a reusable artifact-driven, human-in-the-loop methodology for developing operational metadata schemas from multimodal artifacts, integrating established methods into an evidence-preserving workflow tailored to this input/output setting: multimodal artifacts as evidence and a standards-informed operational metadata schema as the endpoint. The source code is available at https://github.com/ajdajd/placeholderrepo.
