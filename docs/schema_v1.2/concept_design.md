# Data Snapshot Metadata Schema v1.2: Concept Disposition and Design Matrix

Status: Review in progress; Aivin review items remain provisional
Date: 2026-09-04

## Purpose

This document translates the approved semantic baseline in
[`standards_crosswalk.md`](standards_crosswalk.md) into candidate v1.2
application concepts. It is the decision point for field retention, renaming,
definition revision, consolidation, and splitting.

The candidate names and structures below are not yet Pydantic implementation
decisions. They should be approved conceptually before types, cardinalities,
validators, and serialization aliases are finalized.
...
- **Split**: replace an overloaded concept with two or more explicit semantic
  relations.
- **Shape-sensitive**: the concept is retained, but its final attachment or
  cardinality requires representative-snapshot review.
...
## Concept disposition matrix

The examples are minimal JSON-like fragments showing the change from the flat
v1.1.1 field to the candidate v1.2 representation. They illustrate conceptual
...
| v1.1.1 concept | Disposition | Candidate v1.2 location/name | Standards basis | Proposed definition direction | Minimal example | Review note |
|---|---|---|---|---|---|---|
| `title` | Retain | `title` | `dcterms:title`; `schema:name` | The primary title, caption, or heading identifying the data snapshot. | — | Ready. |
| `internal_identifier` | Rename | `document_label` | JATS `label`; `schema:identifier`; `dcterms:identifier` | The label or identifier assigned to the snapshot within its parent source document, such as “Figure 3” or “Table 4.2.” | — | `document_label` better expresses the JATS-like function and avoids implying a globally unique identifier. Confirm name. |
...
| `panel_title` | Rename | `panel_titles` | JATS `fig-group` / `fig` / `caption/title`; `schema:hasPart` + `schema:name` | The ordered titles or headings explicitly shown for individual panels in a multi-panel snapshot. | — | Working default pending Aivin's review. This does not introduce panel objects or infer untitled panels. |

### Subject, dimensions, and presentation

| v1.1.1 concept | Disposition | Candidate v1.2 location/name | Standards basis | Proposed definition direction | Minimal example | Review note |
|---|---|---|---|---|---|---|
| `variable_name` | Rename; fold | `variables[].name` | `schema:variableMeasured`; Schema.org `StatisticalVariable`; DDI Variable; SDMX measure concept | The explicitly named variables, indicators, metrics, or measured concepts represented by the snapshot. | `"variable_name": ["Inflation rate"]` → `"variables": [{"name": "Inflation rate"}]` | Approved variable-centric structure. A variable may carry its own measurement qualifiers without becoming an extracted observation. |
...
| `category_labels` | Fold | `dimensions[].categories` and `dimensions[].category_groups` | SDMX `Code` / `Codelist`; DDI `Category` / `CodeList`; SKOS labels and hierarchical relations | The explicit displayed categories associated with a particular dimension, optionally organized into one broad grouping level and carrying normalized codes or concepts. | `"category_labels": ["Arrestations arbitraires", "Enlèvements"]` → `"dimensions": [{"name": "Incident typology", "presentation_roles": ["column"], "category_groups": [{"name": "Violation du droit à la liberté", "categories": ["Arrestations arbitraires", "Enlèvements"]}]}]` | Approved bounded hierarchy. Preserve ungrouped values in `categories`; use non-recursive `category_groups` only when an explicit two-level grouping is visible. |
| `population_group` | Retain | `population_group` | DDI `Universe`; Schema.org `StatisticalVariable.populationType` | The human population, beneficiary group, or demographic group that is the primary subject of the represented data. | — | Keep distinct from dimensions and categories. Consider a named/controlled-term type later. |
...
| `financing_source` | Rename; fold | `financing.funders` | `schema:funder`; IATI funding-role organization | The organizations or other funding sources explicitly identified as providing financial support. | `"financing_source": ["Asian Development Bank"]` → `"financing": {"funders": [{"name": "Asian Development Bank"}]}` | Candidate role-bearing organization references. |
...
The v1.2 design review is intentionally limited to **18 unique snapshots**:
...
These exceptions do not expand the general design-review sample. They inform
...
Visible image content was treated as the primary evidence in this review;
...
The bounded review supports the dimension, provenance, geographic-coverage,
...
The reviewed Validation 2 hierarchy candidate is resolved within the existing
...
The explicitly reviewed Validation 2 panel candidate demonstrates that flat
...
schema. The working design therefore remains the narrower `panel_titles`
...
deferred for Aivin's review in [`aivin_review.md`](aivin_review.md).

## Proposed review sequence

1. Review the revisions informed by the bounded 18-snapshot review and the two
   explicitly inspected Validation 2 representation-gap examples.
2. Obtain Aivin's input on the questions tracked in
   [`aivin_review.md`](aivin_review.md), including candidate
   application-facing names and the bounded panel-representation decision.
3. Approve the remaining definitions and concept dispositions.
4. Translate the approved design into Pydantic models, then decide exact types,
   cardinalities, validators, and serialization aliases.
5. Freeze v1.2 and run a separate confirmatory Schema v1.2 validation exercise
   using a pre-specified sample and protocol.
