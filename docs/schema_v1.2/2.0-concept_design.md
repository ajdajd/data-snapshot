...
| `comparison_group` | Rename | `comparisons` | Local concept; DDI comparison structures; `schema:measurementQualifier` (related) | The explicit comparative relationships presented by the snapshot, identifying the comparator, benchmark, reference group, cohort, scenario, or entity against which represented data are compared. | `"comparison_group": "Men vs Women"` → `"comparisons": ["Men vs Women"]` | Preserve each complete comparison statement at snapshot level. It is constrained semantic text rather than arbitrary commentary and must not be collapsed into the category list. |
